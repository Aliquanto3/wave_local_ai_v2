...
- Includes: the view-boundary carve-out `the-pitch-opens-on-one-card-per-use-case-read-from-published-rows` needs for its overview card. A card is one visual unit made of two panels: the quality panel reads only the quality overview, the runtime-and-energy panel reads only the runtime overview, and the card shell that places them side by side reads neither response's fields — it only positions the two panels and passes them primitives (a suite name, a list of roster-entry ids), never a quality or runtime value. This is the narrowest reading of "no response that returns both" under which that card can exist: no endpoint's JSON ever carries a field from the other store, and no component that reads one store's response also reads the other's.
...
- Quality and runtime never arrive together — verified by there being no endpoint that returns both, not by a convention about how the pages are laid out. The overview card shell is the one exception to "a convention about layout doesn't count": it is verified structurally too (`views/boundary.test.ts`), since the shell composes both panels' rendering on one card without either response's JSON ever crossing into the other's component.
