...
- Before commit: the fast gate — `ruff check`, `ruff format --check`, `mypy src/ scripts/`, `detect-secrets-hook` — runs as a `pre-commit` stage hook, installed once per clone with `uv run pre-commit install`.
