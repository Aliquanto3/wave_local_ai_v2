...
| If any one counted repetition's `stop_task()` fails to return data, the whole row's energy figures become `unavailable` (all four channel values `None`, `energy_window_method` still names the method that was attempted) rather than a partial sum over fewer repetitions. | Matches `cost.total_or_none`'s existing rule elsewhere in this codebase: a total missing one sample is unknown, not smaller. A partially-summed energy figure silently under-reports without saying so. |
