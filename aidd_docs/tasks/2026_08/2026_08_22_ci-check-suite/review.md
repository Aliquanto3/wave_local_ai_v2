...
- [ ] A deliberately failing PR is red, the same branch turns green once fixed, both states observed — no workflow run exists: the branch is unpushed and `phase-4.md` front matter is still `status: in-progress`
...
| 🟡 | functional | 4 | `aidd_docs/tasks/2026_08/2026_08_22_ci-check-suite/phase-4.md` | Criterion 4 unmet: the falsification the story publishes as its evidence does not exist — no red-then-green run observed. | Push the branch, open the PR, plant a lint violation, capture the red `required` check, revert, capture green; attach both run URLs to the PR description and set `phase-4.md` to `status: done`. |
...
| Unchecked     | Phase 4 criterion 4, a deliberately failing PR observed red then green — fix |
