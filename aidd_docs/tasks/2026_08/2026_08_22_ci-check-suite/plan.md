...
| `https://github.com/astral-sh/setup-uv`                                  | Inputs: `version`, `enable-cache`, `cache-dependency-glob`, `restore-cache`, `save-cache`. There is **no** moving `v10` tag — the repo stopped publishing bare major tags after `v7.6`; the latest release is `v10.0.1` at `20cfd1bf945f4377ade1205e4dbc17946fc9a30d` (verified against the GitHub tags API after CI failed on `@v10`). |
...
| Third-party actions are pinned by commit sha with the version as a trailing comment (`astral-sh/setup-uv@20cfd1bf… # v10.0.1`) | A version tag is a moving target that may not exist at all: `@v10` failed the first run outright ("unable to find version v10") because the publisher does not cut bare major tags. A sha resolves deterministically and cannot be repointed under the workflow. |
