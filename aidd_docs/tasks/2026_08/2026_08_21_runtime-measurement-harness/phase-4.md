...
| 3    | A real (non-stubbed) run of the CLI against the live model produces a row with `gen_tok_per_s` within ±1.5 tok/s of 26 and `prompt_tok_per_s` in the neighborhood of 280 — recorded as the manual verification evidence for this phase, not asserted in an automated test (the project's testing convention forbids tests starting a real server). |
