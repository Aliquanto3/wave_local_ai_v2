...
| `prompt_tok_per_s` acceptance bar re-scoped from "neighborhood of ~280 tok/s" to ~255-260 tok/s for this harness's actual request pattern (fresh `llama-server` launch, one real request). | Investigated per phase-4 task 3.2 (treat the harness as wrong before adjusting the baseline): a live `llama-bench` pp512 run with the harness's exact flags reproduces ~280-315 tok/s, so the hardware/flags aren't the issue. Root-caused in `debug-prefill-gap.md` to two effects specific to this harness's design, both confirmed by live experiment: (1) a short prompt doesn't amortize llama-server's fixed per-request cost (closed by lengthening `FIXED_PROMPT`), and (2) a cold-start tax on the first request served after a fresh model load, which `llama-bench`'s repeated/averaged, never-restarted methodology structurally excludes. A warm-up-request fix for (2) was tried and rejected: it contaminated `gen_tok_per_s` (26 -> 11.8) by leaking context into the measured request's `-np 1` slot. Since the plan's own design requires a fresh server per run (not a long-lived, pre-warmed one), this tax is inherent, not a bug — `gen_tok_per_s` (which has no analogous cold-start component) still matches baseline within ±1.5 tok/s. |
