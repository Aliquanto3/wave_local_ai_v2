...
- Every runtime and quality row now carries a cost block: `cost_total`,
  ...
  run, `list_price_per_million_tokens`/`list_price_currency`/
  `list_price_retrieved_at` for a cloud run — the inapplicable half is
  `null`, never both). Currencies (EUR for local, USD for Mistral's list
  price) are never converted between each other. The writer gate now refuses
  a row whose `cost_total` is non-null but both price bases are null.
  `mistral_client.complete_prompt` now also surfaces `prompt_tokens` and
  `total_tokens` from the response's `usage` block. `SCHEMA_VERSION` moves
  `"4"` → `"5"`.
